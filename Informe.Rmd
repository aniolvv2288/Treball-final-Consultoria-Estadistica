...
Amb l’objectiu de complementar l’anàlisi factorial i els models estadístics tradicionals, es van implementar models d’aprenentatge automàtic supervisat per capturar relacions no lineals i interaccions complexes entre les variables socioeconòmiques i els resultats electorals a nivell de secció censal. En concret, es van utilitzar dos algoritmes basats en conjunts d’arbres de decisió: Random Forest i Extreme Gradient Boosting.

Aquests models van permetre modelar patrons altament no lineals, van mostrar una robustesa relativa davant la multicol·linearitat entre predictors i van facilitar l’anàlisi del paper de les variables socioeconòmiques en la predicció del suport electoral. L’objectiu principal d’aquesta secció va ser identificar quins factors estructurals del territori explicaven millor la variabilitat en el suport als diferents partits polítics.

Els models es van aplicar de manera desagregada per elecció i per candidatura, utilitzant com a variable resposta la quota de vot per secció censal i com a predictors exclusivament indicadors socioeconòmics i demogràfics.
